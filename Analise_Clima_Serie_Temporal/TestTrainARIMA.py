...
class TestTrainARIMA(object):
    path_base = r"C:\Artigos Cientificos\Analise_Historica_MG\Files\Series Temporais\TestTrainARMA"
    ...
    def control_time_series(self):
        ...
        for index in self.dados_clima:
            ...
            list_predict, train_set, test_set = self.train_test_arima(data_climate_format, index)

            self.plot_train_test_arima(list_predict, train_set, test_set, index)
            self.calcute_metrics_arima(list_predict, test_set, index)
    ...
    def calcute_metrics_arima(self, list_predict, test_set, index):
        predict_metrics = []
    ...
    def diagram_correlation(self, list_predict, test_set, index):
        fig, axs = plt.subplots(3, 2, figsize=(14, 10))
        fig.suptitle(f'Correlação dos Resíduos das Séries Temporais ARIMA de {index.title()}', fontsize=16, fontweight='bold')
        ...
        output_path = os.path.join(self.path_base, f"correlacao_{index}_aRIMA.png")
        plt.savefig(output_path)
    ...
    def plot_curve_norm(self, list_predict, list_test_norm_data, index):
        ...
        fig.suptitle(f'Train Test da {index.title()} Mensal - ARIMA', fontsize=13, fontweight='bold')
        ...
        output_path = os.path.join(self.path_base, f"test_norm_{index}_arima.png")
        plt.savefig(output_path)
